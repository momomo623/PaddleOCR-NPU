...
class RecSVTRConverter(BaseOCRV20):
    def __init__(self, config, paddle_pretrained_model_path, **kwargs):
        ...
        super(RecSVTRConverter, self).__init__(config, **kwargs)
...
if __name__ == '__main__':
    ...
    converter = RecSVTRConverter(cfg, paddle_pretrained_model_path, **kwargs)


    inp = torch.from_numpy(np.random.randn(1,3,64,256).astype(np.float32))
    with torch.no_grad():
