...
class LayoutLMModel(LayoutLMPretrainedModel):
    ...
    def forward(
        ...
        if attention_mask is None:
            attention_mask = torch.unsqueeze(torch.unsqueeze(
                (input_ids == self.pad_token_id
                 ).type(self.pooler.dense.weight.dtype) * -1e9,
                dim=1), dim=2)
        else:
        ...
        if bbox is None:
            bbox = torch.zeros(tuple(list(input_shape) + [4]), dtype=torch.long)
...
class LayoutLMForTokenClassification(LayoutLMPretrainedModel):
    ...
    def forward(self,
        ...
        if attention_mask is not None:
            attention_mask = attention_mask.unsqueeze(1).unsqueeze(2).type(torch.long)
