...
# class PretrainedModel(Layer, GenerationMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        ...
        for file_id, file_path in resource_files.items():
            if file_path is None or os.path.isfile(file_path) or os.path.isfile(file_path.replace('.pdparams', '.pth')):
                file_path = file_path.replace('.pdparams', '.pth') if os.path.isfile(file_path.replace('.pdparams', '.pth')) else file_path
                resolved_resource_files[file_id] = file_path
                continue
            path = os.path.join(default_root, file_path.split('/')[-1])
            if os.path.exists(path) or os.path.exists(path.replace('.pdparams', '.pth')):
        ...
        if 'PP-Layout_v1.0_ser_pretrained' in weight_path:
            load_layoutxlm_weights(model, weight_path)
        elif 'PP-Layout_v1.0_re_pretrained' in weight_path:
            load_layoutxlm_weights(model, weight_path)
        ...
        # model_to_load.load_state_dict(state_to_load)
    ...
    def save_pretrained(self, save_dir):
        ...
        # torch.save(model.state_dict(), 'model_state.pth', _use_new_zipfile_serialization=False)
...
def _load_paddle_layoutxlm_weights(torch_model, weights_path):
    ...
    for k, v in load_layer_state_dict.items():
        ...
        if ppname.endswith('.weight'):
            if len(v.shape) == len(cur_w_shape) == 2 \
                    and v.shape[0] == cur_w_shape[1] \
                and v.shape[1] == cur_w_shape[0]:
